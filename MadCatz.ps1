param(
    ...
    [String]$Wallet, 
    [Parameter(Mandatory=$false)]
    [String]$UserName = "MadCatz", 
    [Parameter(Mandatory=$false)]
    [String]$WorkerName = "MadCatz",
    [Parameter(Mandatory=$false)]
    [String]$RigName = "MadCatz",
    [Parameter(Mandatory=$false)]
    [Int]$API_ID = 0, 
    [Parameter(Mandatory=$false)]
    [String]$API_Key = "", 
...
$WalletDonate = "34Qh9LfzaZoxPSwaUzWY9V3Z8n5mLusWQk"
$UserNameDonate = "MadCatz"
$WorkerNameDonate = "MadCatz"
$WalletBackup = $Wallet
...
while($true)
{
    ...
    try {
        Write-Host "MadCatz checking for updates from Coinbase..." -b "Magenta" -foregroundcolor "Cyan"
    ...
    Write-host "tries MadCatz at Cryptomining.." -foregroundcolor "Cyan"
    ...
    $ActiveMinerPrograms | Sort -Descending Status,{if($_.Process -eq $null){[DateTime]0}else{$_.Process.StartTime}} | Select -First (1+6+6) | Format-Table -Wrap -GroupBy Status (
        ...
        Write-Host "                              MadCatz Version 1" -f "Cyan"                                
        Write-Host ""
        Write-Host "                                 $(Get-Date -Format "dd / MM / yyyy")"  -foregroundcolor "White"
        Write-Host ""
        Write-Host "                       Thank you Everyone For using MadCatz!!!!!" -foregroundcolor "Magenta"
        Write-Host ""
        Write-Host ""
        Write-Host ""
        Write-Host ""
        Write-Host "     1BTC = " $Rates.$Currency "$Currency" -Background "Red" -foregroundcolor "Yellow"
    ...
    if (($BestMiners_Combo | Where-Object Profit -EQ $null | Measure-Object).Count -eq 0) {
        $MinerComparisons = 
        [PSCustomObject]@{"Miner" = "MadCatz.. Maddd out!"}, 
        ...
        if ($MinerComparisons_Profit[0] -gt $MinerComparisons_Profit[1]) {
            ...
            Write-Host -BackgroundColor Magenta -ForegroundColor Cyan "MadCatz Maddd $([Math]::Round((((($MinerComparisons_Profit[0]-$MinerComparisons_Profit[1])/$MinerComparisons_Profit[1])-$MinerComparisons_Range)*100)))% and upto $([Math]::Round((((($MinerComparisons_Profit[0]-$MinerComparisons_Profit[1])/$MinerComparisons_Profit[1])+$MinerComparisons_Range)*100)))% more profit than the fastest (listed) miner: "
        }
    ...
    $ActiveMinerPrograms | ForEach {
        ...
        else
        {
            ...
            if($Miner_HashRates.Count -ge $_.Algorithms.Count)
            {
                ...
                Write-Host "MadCatz chews on"$_.Algorithms" then saves hashrate" -foregroundcolor "Cray"
            }
        }
    }
